...
def load_train_config(trainConfigPath):
    ...
    with open(trainConfigPath, "r") as f:
        config = json.load(f)
    train_config = CustomCoupledHierarchicalTransformerTrainConfig(**config)
    return train_config
...
def train_custom_cht(train_config: CustomCoupledHierarchicalTransformerTrainConfig):
    ...
    tokenizer = BertTokenizer.from_pretrained(
        train_config.bert_model, do_lower_case=train_config.do_lower_case
    )

    train_data, eval_data, test_data, num_train_steps = prepare_data_for_training(
        processor, stance_processor, tokenizer, train_config
    )
    ...
    if train_config.do_train:
        ...
        train_dataloader = DataLoader(
            train_data, sampler=train_sampler, batch_size=train_config.train_batch_size
        )

        # Run prediction for full data
        eval_sampler = SequentialSampler(eval_data)
        eval_dataloader = DataLoader(
            eval_data, sampler=eval_sampler, batch_size=train_config.eval_batch_size
        )

        # Run prediction for full data
        test_sampler = SequentialSampler(test_data)
        test_dataloader = DataLoader(
            test_data, sampler=test_sampler, batch_size=train_config.eval_batch_size
        )
    ...
    if train_config.do_eval and (
        ...
        eval_examples = processor.get_test_examples(train_config.data_dir)
