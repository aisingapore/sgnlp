...
class TestUFDUtilsTestCase(unittest.TestCase):
    def setUp(self) -> None:
        cfg = {
            ...
            "model_folder": "https://sgnlp.blob.core.windows.net/models/ufd",
    ...
    def test_load_trained_models(self):
        ufd_ad, ufd_ag, ufd_cfm, ufd_cls = load_trained_models(self.cfg, 'books', 'de', 'music')
        self.assertEqual(ufd_ad.__class__, UFDAdaptorDomainModel)
        self.assertEqual(ufd_ag.__class__, UFDAdaptorGlobalModel)
        self.assertEqual(ufd_cfm.__class__, UFDCombineFeaturesMapModel)
    ...
    def test_create_classifiers(self):
        ...
        self.assertEqual(len(classifiers.keys()), len(self.cfg.train_args['source_domains']))
    ...
    def test_get_source2target_domain_mapping(self):
        ...
        self.assertEqual(len(mappings.keys()), len(self.cfg.train_args['source_domains']))
    ...
    def test_generate_train_val_dataset(self):
        ...
        self.assertEqual(list(valid_data.keys()).sort(), self.cfg.train_args['target_languages'].sort())
        ...
        for k, v in valid_data.items():
            ...
            for k1, v1 in valid_data[k].items():
                ...
                self.assertEqual(len(valid_data[k]), len(self.cfg.train_args['target_domains']))
        ...
        self.assertEqual((str(train_cache_path), train_cache_path.is_file()), (str(train_cache_path), True))
        self.assertEqual((str(valid_cache_path), valid_cache_path.is_file()), (str(valid_cache_path), True))
